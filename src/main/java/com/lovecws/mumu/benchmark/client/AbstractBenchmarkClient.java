...
package com.lovecws.mumu.benchmark.client;

import com.lovecws.mumu.benchmark.statistics.BenchmarkStatistics;
import com.lovecws.mumu.benchmark.statistics.RunnableStatistics;
...
public abstract class AbstractBenchmarkClient {
    ...
    public static final int WARMUPTIME = 30;//项目预热时间
    ...
    public void start(int concurrents, int runtime, String classname, String params) {
        ...
        List<RunnableStatistics> runnableStatisticses = new ArrayList<RunnableStatistics>();
        for (ClientRunnable runnable : runnables) {
            runnableStatisticses.add(runnable.getStatistics());
        }
        statistics = new BenchmarkStatistics(runnableStatisticses);
